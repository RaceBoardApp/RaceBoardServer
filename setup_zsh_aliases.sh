...
# Function to add content to .zshrc if not already present
add_to_zshrc() {
    local content="$1"
    if ! grep -Fq "$content" "$HOME/.zshrc"; then
...
# Add Raceboard section marker
echo "" >> "$HOME/.zshrc"
add_to_zshrc "# ===== Raceboard Server Configuration ====="

# Add PATH for release binaries
if add_to_zshrc "export PATH=\"$RELEASE_DIR:\$PATH\""; then
    echo -e "${GREEN}✓ Added release binaries to PATH${NC}"
else
    echo -e "${YELLOW}! Release binaries already in PATH${NC}"
fi

# Add aliases for main server
if add_to_zshrc "alias raceboard-server='$RELEASE_DIR/raceboard-server'"; then
    echo -e "${GREEN}✓ Added raceboard-server alias${NC}"
fi

if add_to_zshrc "alias raceboard-server-dev='$DEBUG_DIR/raceboard-server'"; then
    echo -e "${GREEN}✓ Added raceboard-server-dev alias (debug build)${NC}"
fi

# Add aliases for adapters
if add_to_zshrc "alias raceboard-gitlab='$RELEASE_DIR/raceboard-gitlab'"; then
    echo -e "${GREEN}✓ Added raceboard-gitlab alias${NC}"
fi

if add_to_zshrc "alias raceboard-cmd='$RELEASE_DIR/raceboard-cmd'"; then
    echo -e "${GREEN}✓ Added raceboard-cmd alias${NC}"
fi

if add_to_zshrc "alias raceboard-claude='$RELEASE_DIR/raceboard-claude'"; then
    echo -e "${GREEN}✓ Added raceboard-claude alias${NC}"
fi

if add_to_zshrc "alias raceboard-codex='$RELEASE_DIR/raceboard-codex'"; then
    echo -e "${GREEN}✓ Added raceboard-codex alias${NC}"
fi

if add_to_zshrc "alias raceboard-gemini-watch='$RELEASE_DIR/raceboard-gemini-watch'"; then
    echo -e "${GREEN}✓ Added raceboard-gemini-watch alias${NC}"
fi

if add_to_zshrc "alias raceboard-track='$RELEASE_DIR/raceboard-track'"; then
    echo -e "${GREEN}✓ Added raceboard-track alias${NC}"
fi
...
raceboard-gitlab-start() {
    local config_file=\"\${1:-$SCRIPT_DIR/gitlab_config.toml}\"
    ...
    $RELEASE_DIR/raceboard-gitlab --config \"\$config_file\" &
...
race() {
    ...
    $RELEASE_DIR/raceboard-cmd start --title \"\$@\"
}
...
race-with-prompt() {
    ...
    shift
    local prompt=\"\$@\"
    $RELEASE_DIR/raceboard-cmd start --title \"\$title\" --prompt \"\$prompt\"
}

race-pass() {
    # Mark the last race as passed
    local race_id=\$($RELEASE_DIR/raceboard-cmd list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
    if [ -n \"\$race_id\" ]; then
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --state passed
...
race-fail() {
    ...
    local race_id=\$($RELEASE_DIR/raceboard-cmd list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
    if [ -n \"\$race_id\" ]; then
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --state failed
...
race-cancel() {
    ...
    local race_id=\$($RELEASE_DIR/raceboard-cmd list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
    if [ -n \"\$race_id\" ]; then
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --state canceled
...
race-list() {
    # List recent races
    $RELEASE_DIR/raceboard-cmd list \"\$@\"
}
...
race-progress() {
    ...
    local race_id=\$($RELEASE_DIR/raceboard-cmd list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
    if [ -n \"\$race_id\" ]; then
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --progress \"\$progress\"
...
race-eta() {
    ...
    local race_id=\$($RELEASE_DIR/raceboard-cmd list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
    if [ -n \"\$race_id\" ]; then
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --eta \"\$eta\"
...
race-event() {
    ...
    local race_id=\$($RELEASE_DIR/raceboard-cmd list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
    if [ -n \"\$race_id\" ]; then
        $RELEASE_DIR/raceboard-cmd event \"\$race_id\" --message \"\$event\"
...
track() {
    local cmd=\"\$@\"
    local race_id=\$($RELEASE_DIR/raceboard-cmd start --title \"Command: \$cmd\" --source \"shell\" 2>/dev/null | grep -oE 'cmd:[0-9]+')
    
    # Execute the command
    eval \"\$cmd\"
    local exit_code=\$?
    
    # Update race based on exit code
    if [ \$exit_code -eq 0 ]; then
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --state passed 2>/dev/null
        echo \"✓ Command completed successfully (tracked as \$race_id)\"
    else
        $RELEASE_DIR/raceboard-cmd update \"\$race_id\" --state failed 2>/dev/null
        echo \"✗ Command failed with exit code \$exit_code (tracked as \$race_id)\"
    fi
...
echo -e "${GREEN}✓ Added helper functions${NC}"

# Add completion setup
add_to_zshrc "
# Raceboard command completion
_raceboard_completion() {
    local commands=(
        'start:Start the Raceboard server'
        'stop:Stop the Raceboard server'
        'restart:Restart the Raceboard server'
...
compdef _raceboard_completion raceboard-
"
